...
<div class="layui-tab">
    <div class="layui-tab-content">
        <div class="layui-tab-item layui-show">
            <jsp:include page="TAppProjectAssessSearch.jsp"></jsp:include>
        </div>
    </div>
</div>
...
<script>
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function () {
        ...
        table.render({
            elem: '#table',//table id
            url: '<%=request.getContextPath()%>/tAppProjectAssess/datagrid',
            method: 'POST', //方式
            page: true,//是否开启分页
            ...
            cols: [[ //标题栏
                {checkbox: true},
				{field: 'pprojectid', title: '项目ID',align: 'center', sort: true},
				{field: 'pcompleterate', title: '项目完成率',align: 'center', sort: true},
				{field: 'pfinishhour', title: '实际工时',align: 'center', sort: true},
				{field: 'pfinishhourrate', title: '实际工时比计划工时',align: 'center', sort: true},
				{field: 'punfinishhour', title: '未完成工时',align: 'center', sort: true},
        ...
        function openAdd() {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TAppProjectAssessAdd.jsp', 'yes'], //iframe的url，no代表不显示滚动条
        ...
        function openDetail(data) {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TAppProjectAssessDetail.jsp?obj=' + encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success: function (layero, lockIndex) {
                    var body = layer.getChildFrame('body', lockIndex);
                    //绑定解锁按钮的点击事件
                    body.find('button#close').on('click', function () {
        ...
        function openUpdate(data) {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TAppProjectAssessEdit.jsp?obj=' + encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success: function (layero, lockIndex) {
            ...
            layer.open({
                ...
                , success: function (layero) {
                    ...
                    btn.find('.layui-layer-btn0').on('click', function () {
                        ...
                        $.ajax({
                            url: '<%=request.getContextPath()%>/tAppProjectAssess/remove',
                            data: {
                    id :
                        ids
                    },
