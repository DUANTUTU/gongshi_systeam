...
<div class="layui-tab">
    <div class="layui-tab-content">
        <div class="layui-tab-item layui-show">
            <jsp:include page="TAppManhourConfirmSearch.jsp"></jsp:include>
        </div>
    </div>
</div>
...
<script>
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function () {
        ...
        table.render({
            elem: '#table',//table id
            url: '<%=request.getContextPath()%>/tAppManhourConfirm/datagrid',
            method: 'POST', //方式
            page: true,//是否开启分页
            ...
            cols: [[ //标题栏
                {checkbox: true},
                {field: 'mprojectidNm', title: '项目', align: 'center', sort: true},
        ...
        function openUpdate(data) {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TAppManhourConfirmEdit.jsp?obj=' + encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success: function (layero, lockIndex) {
            ...
            layer.open({
                ...
                , success: function (layero) {
                    ...
                    btn.find('.layui-layer-btn0').on('click', function () {
                        ...
                        $.ajax({
                            url: '<%=request.getContextPath()%>/tAppManhourConfirm/remove',
                            data: {
                                id:
                                ids
                            },
...
<script type="text/html" id="toobar">
    ...
    <a class="layui-btn layui-btn-xs {{ d.mcheckstatus != 2 ? '' : 'layui-btn-disabled' }} "
       lay-event="{{ d.mcheckstatus != 2 ? 'update' : '' }}">修改</a>
    <a class="layui-btn layui-btn-danger layui-btn-xs {{ d.mcheckstatus != 2 ? '' : 'layui-btn-disabled' }} "
       lay-event="{{ d.mcheckstatus != 2 ? 'del' : '' }}">删除</a>
</script>
