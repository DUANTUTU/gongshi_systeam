...
<script type="text/javascript">
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function(){
        ...
        //授权功能
        function sys_RoleInf_RoleInfEast_save(projectId) {
            ...
            for ( var i = 0; i < checkedNodes.length; i++) {
                if (checkedNodes[i].getCheckStatus().checked
                    && "person"==checkedNodes[i].alias) {
                    checkArray.push(checkedNodes[i].id+"."+checkedNodes[i].pid);
                }
            }
            ...
            $.ajax({
                url : '<%=request.getContextPath()%>/tAppProjectPerson/addPersonByProject',
                data : {
                    operCds : checkAppList,
                    projectId : projectId
                },
                dataType : 'json',
                success : function(r) {
                    ...
                    //初始化树
                    sys_RoleInf_RoleInfEast_init(projectId);
        ...
        sys_RoleInf_RoleInfEast_init(data.id);//初始化树

        //选中树
        function sys_RoleInf_RoleInfEast_checkTree(projectId) {
            var treeObj = $.fn.zTree.getZTreeObj("sys_RoleInf_RoleInfEast_ztree");

            $.ajax({
                url : '<%=request.getContextPath()%>/tAppProjectPerson/getPersonByProjectId',
                data : {
                    projectId : projectId
                },
                type : 'POST',
                ...
                success : function(r) {
                    ...
                    for ( var i = 0; i < rows.length; i++) {
                        var node = treeObj.getNodeByParam("id", rows[i],
                            null);
                        if (node) {
        ...
        //初始树
        function sys_RoleInf_RoleInfEast_init(projectId) {
            var index = layer.load(1);//开启进度条
            $.ajax({
                url : "<%=request.getContextPath()%>/tAppProject/getInsPersonTree",
                type : 'POST',
                ...
                success : function(data) {
                    ...
                    if (projectId != '') {
                        sys_RoleInf_RoleInfEast_checkTree(projectId);
                    }
                },
                error : function(msg) {
                    layer.close(index);//关闭
        ...
        $('button#btnSubmit').on('click', function() {
            sys_RoleInf_RoleInfEast_save(data.id);
        });
        ...
        $('button#reSet').on('click', function() {
            ...
            //初始化树
            sys_RoleInf_RoleInfEast_init(data.id);
        });
