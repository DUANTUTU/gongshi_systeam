...
<div class="layui-tab">
    <div class="layui-tab-content">
        <div class="layui-tab-item layui-show">
            <jsp:include page="TSysJobSearch.jsp"></jsp:include>
        </div>
    </div>
</div>
...
<script>
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function () {
        ...
        table.render({
            elem: '#table',//table id
            url: '<%=request.getContextPath()%>/tSysJob/datagrid',
            method: 'POST', //方式
            page: true,//是否开启分页
            ...
            cols: [[ //标题栏
                {checkbox: true},
				{field: 'jname', title: '职位名称',align: 'center', sort: true},
				{field: 'jtypeNm', title: '职位类型',align: 'center',templet: '#jtypeTpl'},
				{field: 'jstatusNm', title: '职位状态',align: 'center',templet: '#jstatusTpl'},
				{field: 'jcreatedate', title: '创建时间', align: 'center', sort: true},
        ...
        function openAdd() {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TSysJobAdd.jsp', 'yes'], //iframe的url，no代表不显示滚动条
        ...
        function openDetail(data) {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TSysJobDetail.jsp?obj=' + encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success: function (layero, lockIndex) {
                    var body = layer.getChildFrame('body', lockIndex);
                    //绑定解锁按钮的点击事件
                    body.find('button#close').on('click', function () {
        ...
        function openUpdate(data) {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TSysJobEdit.jsp?obj=' + encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success: function (layero, lockIndex) {
            ...
            layer.open({
                ...
                , success: function (layero) {
                    ...
                    btn.find('.layui-layer-btn0').on('click', function () {
                        ...
                        $.ajax({
                            url: '<%=request.getContextPath()%>/tSysJob/remove',
                            data: {
                    id :
                        ids
                    },
...
<script type="text/html" id="jstatusTpl">
    <span class="layui-badge  {{ d.jstatus == 0 ? 'layui-bg-green' : '' }} ">{{d.jstatusNm}}</span>
</script>
<script type="text/html" id="jtypeTpl">
    <span class="layui-badge  {{ d.jtype == 0 ? 'layui-bg-green' : '' }} ">{{d.jtypeNm}}</span>
</script>
</body>
</html>
