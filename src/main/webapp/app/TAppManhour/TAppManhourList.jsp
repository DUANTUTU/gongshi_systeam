...
<div class="layui-tab">
    <div class="layui-tab-content">
        <div class="layui-tab-item layui-show">
            <jsp:include page="TAppManhourSearch.jsp"></jsp:include>
        </div>
    </div>
</div>
...
<script>
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function () {
        ...
        table.render({
            elem: '#table',//table id
            url: '<%=request.getContextPath()%>/tAppManhour/datagrid',
            method: 'POST', //方式
            page: true,//是否开启分页
            ...
            cols: [[ //标题栏
				...
				// {field: 'mmanhourconfimid', title: '确认工时填报id',align: 'center', sort: true},
				{field: 'mprojectidNm', title: '项目名称',align: 'center', sort: true},
				{field: 'mprojectplanidNm', title: '项目里程碑',align: 'center', sort: true},
				{field: 'mworkdetails', title: '工作内容',align: 'center', sort: true},
				{field: 'mmanhour', title: '确认工时',align: 'center', sort: true},
				{field: 'mopercdNm', title: '填报人ID',align: 'center', sort: true},
        ...
        function openAdd() {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TAppManhourAdd.jsp', 'yes'], //iframe的url，no代表不显示滚动条
        ...
        function openDetail(data) {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TAppManhourDetail.jsp?obj=' + encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success: function (layero, lockIndex) {
                    var body = layer.getChildFrame('body', lockIndex);
                    //绑定解锁按钮的点击事件
                    body.find('button#close').on('click', function () {
        ...
        function openUpdate(data) {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TAppManhourEdit.jsp?obj=' + encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success: function (layero, lockIndex) {
            ...
            layer.open({
                ...
                , success: function (layero) {
                    ...
                    btn.find('.layui-layer-btn0').on('click', function () {
                        ...
                        $.ajax({
                            url: '<%=request.getContextPath()%>/tAppManhour/remove',
                            data: {
                    id :
                        ids
                    },
        ...
        $('#btnExport').on('click', function () {
            ...
            $.ajax({
                url: '<%=request.getContextPath()%>/tAppManhour/datagridExcl',
                data: JSON.stringify(searchform),
                type: 'POST',//默认以get提交，以get提交如果是中文后台会出现乱码
                dataType: 'json',
                success: function (obj) {
                    if (obj.success) {
                        window.location.href = "<%=request.getContextPath()%>/tAppManhour/exportManhour";
                    } else {
                        layer.alert(obj.msg)
                    }
                }
