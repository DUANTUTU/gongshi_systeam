...
<script>
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function () {
        ...
        table.render({
            elem: '#table',//table id
            url: '<%=request.getContextPath()%>/tAppMeritTemplate/datagrid',
            method: 'POST', //方式
            page: true,//是否开启分页
            ...
            cols: [[ //标题栏
				...
				{field: 'mjobidNm', title: '职位',align: 'center', sort: true},
				{field: 'mmanhour', title: '工时设定',align: 'center', sort: true},
				{field: 'mtarget', title: '任务目标',align: 'center', sort: true},
				{field: 'mdetails', title: '任务内容',align: 'center', sort: true},
				{field: 'mislinkprojectNm', title: '是否关联项目',align: 'center', templet:'#mislinkprojectTpl'},
				{field: 'mcreatedate', title: '创建时间',align: 'center', sort: true},
				{field: 'mupdatedate', title: '修改时间', align: 'center', sort: true},
        ...
        function openAdd() {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TAppMeritTemplateAdd.jsp', 'yes'], //iframe的url，no代表不显示滚动条
        ...
        function openDetail(data) {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TAppMeritTemplateDetail.jsp?obj=' + encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success: function (layero, lockIndex) {
                    var body = layer.getChildFrame('body', lockIndex);
                    //绑定解锁按钮的点击事件
                    body.find('button#close').on('click', function () {
        ...
        function openUpdate(data) {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['TAppMeritTemplateEdit.jsp?obj=' + encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success: function (layero, lockIndex) {
            ...
            layer.open({
                ...
                , success: function (layero) {
                    ...
                    btn.find('.layui-layer-btn0').on('click', function () {
                        ...
                        $.ajax({
                            url: '<%=request.getContextPath()%>/tAppMeritTemplate/remove',
                            data: {
                    id :
                        ids
                    },
...
<script type="text/html" id="mislinkprojectTpl">
    <span class="layui-badge  {{ d.mislinkproject == 0 ? 'layui-bg-green' : '' }} ">{{d.mislinkprojectNm}}</span>
</script>
</body>
</html>
