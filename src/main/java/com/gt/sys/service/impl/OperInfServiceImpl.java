...
public class OperInfServiceImpl extends BaseServiceImpl<TSysOperInf> implements IOperInfService {
    ...
    public DatagridForLayUI datagrid(OperInf operInf, String writeOperInsCd, boolean v) throws Exception {
        ...
        String sqlleft = "select * from ( select a.oper_cd as operCd, a.job, a.oper_Nm, a.email,  b.INS_DETAIL_NOT_MY ,  a.oper_nm as operNm,  a.oper_st as operSt,  a.telephone, a.ins_cd as insCd, b.ins_nm as insCdNm ,xs.DICT_NM as operStNm,f.DICT_NM as jobNm,a.sjobid as sjobid,g.JName as sjobidNm ";
//        sqlleft += " ,GROUP_CONCAT(DISTINCT(c.ROLE_CD)) as roleCd,GROUP_CONCAT(DISTINCT(d.ROLE_NM)) as roleCdNm";
        sqlleft += " ,c.ROLE_CD as roleCd,d.ROLE_NM as roleCdNm";
        ...
        sql += " left join t_sys_job g on a.sjobid=g.ID";
        sql += "  ) as a where 1=1 ";
    ...
    @Override
    public List<Map> getAllListByInsCd(String inscd) {
        List<Map> tl = new ArrayList<>();
        String sql = "select * from t_sys_oper_inf where ins_cd ='"+ inscd + "'";
        String hql = " from TSysOperInf t where ins_Cd='" + inscd + "'";
        tl = findBySql(sql);
