...
public class InsInfServiceImpl extends BaseServiceImpl<TSysInsInf> implements IInsInfService {
    ...
    public Json add(InsInf inf) {
        ...
        if (!PbUtils.isEmpty(inf.getParentInsCd())) {
            ...
            if (6 <= Integer.valueOf(tSysInsInf.getInsLvl())) {
                j.setSuccess(false);
                j.setMsg("新增失败：该机构下无法增加子级,机构已达到最大层级限制");
                return j;
            }
            tInf.setTSysInsInf(tSysInsInf);
            tInf.setInsLvl(String.valueOf(Integer.valueOf(tSysInsInf.getInsLvl() == null || "".equals(tSysInsInf.getInsLvl()) ? "0" : tSysInsInf.getInsLvl()) + 1));
...
//            tInf.setTSysInsInf(tSysInsInf);
    ...
    @Override
    public List<TreeForLayUI> getInsListForLayUINew(String insId, String insTp) {
        List<TreeForLayUI> zForLayUIs = new ArrayList<TreeForLayUI>();

        String sql = "SELECT TRIM(UUID) AS ID,TRIM(INS_CD) INSCD ,TRIM(INS_NM) AS NAME,TRIM(PARENT_INS_CD) AS PID,TRIM(INS_LVL) AS LVL FROM t_sys_ins_inf  t";
        sql += "	where  t.INS_DETAIL like:insCd";

        Map<String, Object> params = new HashMap<String, Object>();
        params.put("insCd", "%%" + insId + "%%");
...
//            if ("0".equals(entityMap.get("LVL").toString())) {
//                TreeForLayUI treeForLayUI = getTree(entityMap, list);
//                if (null == treeForLayUI) {
//                    continue;
//                }
//                zForLayUIs.add(treeForLayUI);
//            }
//        }
//        if (zForLayUIs.size() <= 0) {
        for (Map entityMap : list) {
            if (insId.equals(entityMap.get("ID").toString())) {
                TreeForLayUI treeForLayUI = getTree(entityMap, list);
                if (null == treeForLayUI) {
                    continue;
                }
...
//        }
    ...
    //递归检索根节点下所有child
    private TreeForLayUI getTree(Map parentNodeMap, List<Map> list) {
        TreeForLayUI treeForLayUI = new TreeForLayUI();
        treeForLayUI.setId(parentNodeMap.get("ID").toString());
        treeForLayUI.setName(parentNodeMap.get("NAME").toString());
        treeForLayUI.setPid(parentNodeMap.get("PID") == null ? "" : parentNodeMap.get("PID").toString());
        if ("0".equals(parentNodeMap.get("LVL").toString())) {
            treeForLayUI.setSpread(true);
        } else {
            treeForLayUI.setSpread(false);
        }
        List<TreeForLayUI> listTree = new ArrayList<>();
        for (Map entityMap : list) {
            if (treeForLayUI.getId().equals(entityMap.get("PID"))) {
                TreeForLayUI childNode = getTree(entityMap, list);
                listTree.add(childNode);
            }
            treeForLayUI.setChildren(listTree);
        }
        return treeForLayUI;
    }
